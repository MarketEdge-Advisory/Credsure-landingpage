...
const LoanCalculator = () => {
  ...
  // Auto-populate when a car is selected
  useEffect(() => {
    if (selectedCar) {
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 max-w-6xl mx-auto">
          ...
          <div className="bg-[#FFFFFF0D] rounded-none p-4 sm:p-6 md:p-8 shadow-xl">
            <h3 className="text-white text-lg sm:text-xl font-bold mb-4 sm:mb-6 uppercase tracking-wider">Loan Calculator</h3>

            {/* Vehicle Select */}
            <div className="mb-4 sm:mb-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-1 sm:gap-0">
                <label className="text-gray-300 text-xs sm:text-sm font-medium">Select Vehicle Model</label>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const carObj = cars.find(c => c.name.trim().toLowerCase() === e.target.value.trim().toLowerCase());
                    setCarId(carObj?.id || '');
            ...
            {/* Loan Tenure */}
            <div className="mb-4 sm:mb-6">
              <label className="text-gray-300 text-xs sm:text-sm font-medium mb-2 block">Loan Tenure</label>
              <div className="relative">
                <select
                  value={loanTenure}
            ...
            {/* Down Payment */}
            <div className="mb-0">
              <label className="text-gray-300 text-xs sm:text-sm font-medium mb-2 block">Advance Payment (₦)/Down Payment (₦)</label>
              <input
                type="text"
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
            <div className="bg-white rounded-lg max-w-2xl w-full relative max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <button
                    ...
                    {showEmploymentDropdown && (
                      <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                        {['Salaried Employee', 'Business Owner', 'Self Employed', 'Corporate'].map(option => (
                          <div key={option} onClick={() => { setFormData({...formData, employmentStatus: option}); setShowEmploymentDropdown(false); }} className="px-4 py-4 text-base text-gray-900 hover:bg-gray-50 cursor-pointer transition-colors">{option}</div>
                        ))}
                      </div>
                    )}
                  </div>
